...
<section data-gsap-anim="section" @if(!empty($section_id)) id="{{ $section_id }}" @endif class="s-hero-prod relative pt-20 {{ $sectionClass }} {{ $section_class }}">
	...
	<div class="__wrapper w-full lg:w-4/5">
		...
		<div class="__content w-full sm:w-3/4  z-10">
			...
			@if (!empty($g_hero_prod['txt']))
			<p data-gsap-element="txt" class="text-xl mt-4 mb-8">{{ strip_tags($g_hero_prod['txt']) }}</p>
			@endif

			<a data-gsap-element="arrow" href="#" class="__scroll js-scroll-next block w-max">
	...
	<img class="absolute -top-170 -right-150" src="{{ $g_hero_prod['image']['url'] }}" alt="{{ $g_hero_prod['image']['alt'] ?? '' }}">
